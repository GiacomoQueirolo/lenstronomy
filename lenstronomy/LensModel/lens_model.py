...
class LensModel(object):
    ...
    def __init__(self, lens_model_list, z_lens=None, z_source=None, lens_redshift_list=None, cosmo=None,
                 ...
                 kwargs_interp=None):#,parallel_single_plane=False):
        """
        ...
        :param num_z_interp: (only in multi-plane with cosmo_interp=True); number of redshift bins for interpolating
         distances
        :param parallel_single_plane: bool, if True and the lens_model is SinglePlane, will instead use ParallelSinglePlane 
         to speed up computation when considering many (>1e6) lenses in the plane (e.g. particles)
        """
        ...
        if multi_plane is True:
            ...
            if los_effects is True:
                ...
                self.lens_model = SinglePlane(lens_model_list,
                    ...
                    kwargs_interp=kwargs_interp)
                """else:
                    self.lens_model = ParallelSinglePlane(lens_model_list,
                        numerical_alpha_class=numerical_alpha_class,
                        lens_redshift_list=lens_redshift_list,
                        z_source_convention=z_source_convention,
                        kwargs_interp=kwargs_interp)"""
...
# WIP
"""
class ParallelLensModel(LensModel):
    def __init__(self,  lens_model_list, z_lens=None, z_source=None, lens_redshift_list=None, cosmo=None,numerical_alpha_class=None, 
                 observed_convention_index=None,z_source_convention=None,kwargs_interp=None):
        super().__init__(lens_model_list=lens_model_list, z_lens=z_lens, z_source=z_source, lens_redshift_list=lens_redshift_list, cosmo=cosmo,
                 multi_plane=False, numerical_alpha_class=numerical_alpha_class, observed_convention_index=None,
                 z_source_convention=z_source_convention, cosmo_interp=False,
                 z_interp_stop=None, num_z_interp=None,
                 kwargs_interp=kwargs_interp)
